...
public class DfaImpl implements TokenMatcher {
    ...
    private static void collapse(final StateNetworkImpl network, final StateImpl keep, final StateImpl remove) {
        if (LOG.isDebugEnabled()) {
            LOG.debug("collapse keep {} remove {}", network.getStates().indexOf(keep), network.getStates().indexOf(remove));
        }
    ...
    /**
     * Empty transitions from any state to remove will be redirected to keep. Except empty transitions from keep.
     */
